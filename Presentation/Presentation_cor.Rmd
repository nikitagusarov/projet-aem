...
subtitle: "Analyse empirique des marchÃ©s"
author: A. Blanc, N. Gusarov, S. Picon
shortauthor : A.Blanc, N.Gusarov, S.Picon
institute: UniversitÃ© Grenoble Alpes
shortinstitute: UGA
date: 19/12/2019
header-includes:
    - \usepackage{array}
...
- Présentation de la problÃ©matique
    - Pesticides 
    - Marché du vin 
...
## Présentation du problème des pesticides
- Source de nombreux débats sur la santÃ© et l'environnement.
- Le rôle actuel :
    - Moyen de protection contre les aléas climatiques ;
    - Outil pour la préservation du rendement.

- Plusieurs mesures mises en places pour rÃ©duire leurs usages :
...
- Les grandes cultures (blés, etc...) sont les premiÃ¨res utilisatrices de pesticides 67.4% ;
- Les vignes sont les deuxièmes 14.4% (Butault et al, 2011).
    
## Comment diminuer l'utilisation de pesticides
Les méthodes contemporaines visant à réduire l'utilisation des pesticides sont :
...
## Les vins de table
Les vins de table sont des vins sans indication gÃ©ographique :

- hétérogénéité plus faible que pour les autre types ;
- prix bas.

Nous traitons seulement des vins sans indication gÃ©ographique :

- La situation sur ce marché influence l'utilisation des pesticides ;
- Il existe une homogénéité presque parfaite parmi les vins sans indication géographique (Cembalo et al, 2014).
...
- On observe l'équilibre sur le marché au niveau du pays. Ainsi, la quantité demandée = quantité offerte par l'ensemble des départements.
...
- Variation par département français (pour les départements produisant le vin)
- Variation par année (2012 à 2016)
   
## Description des données :
- Toutes les variables varient par département et par année.
- La période temporelle comprise dans notre échantillon est de 2012 à 2016.
...
## Visualisatoin des interdÃ©pendances
```{r echo = FALSE}
p1 = datai %>% 
    ...
    # ggtitle("Q ~ IP") +
    xlab("QuantitÃ© du vin") + ylab("Index du prix") +
    pres_theme
p2 = datai %>% 
    ...
    geom_smooth(method = "lm", se = F, size = 0.25) +
    # ggtitle("Q ~ IK") +
    xlab("QuantitÃ© du vin") + ylab("Index des pÃ©sticides") +
    pres_theme
...
p1 = datai %>% 
    ...
    geom_smooth(method = "lm", se = F, size = 0.25) +
    # ggtitle("Q ~ S") +
    xlab("QuantitÃ© du vin") + ylab("Surface qultivÃ©") +
    pres_theme
p2 = datai %>% 
    ...
    geom_smooth(method = "lm", se = F, size = 0.25) +
    # ggtitle("Q ~ R") +
    xlab("QuantitÃ© du vin") + ylab("Revenus") +
    pres_theme
grid.arrange(p1, p2, nrow = 1)
```
...
colnames(correlation) = c("QuantitÃ© du vin", "IP", 
        "Surface", "Revenus", 
        "Index pÃ©sticides", "Temps")
rownames(correlation) = c("QuantitÃ© du vin", "IP", 
        "Surface", "Revenus", 
        "Index pÃ©sticides", "Temps")
```
...
p2 = dataW %>% 
    ...
    # ggtitle("Q ~ IK") +
    xlab("Quantité du vin") + ylab("Index des pesticides") +
    pres_theme
grid.arrange(p1, p2, nrow = 1)
# Probably it is better to do it by variable
```
...
p1 = dataW %>% 
    ...
    # ggtitle("Q ~ S") +
    xlab("Quantité du vin") + ylab("Surface cultivée") +
    pres_theme
p2 = dataW %>% 
    ggplot(aes(qi, ri, col = as.factor(ndep))) +
    geom_point(size = 0.5) + 
...
- Limites du modèle 
    - Faible représentation des effets hétérogènes entre les régions (nous estimons seulement des effets moyens)
...
```{r include = FALSE, eval = FALSE}
# - VÃ©rification des hypothÃ¨ses (5 hypothÃ¨ses) :
#     - La moyenne nulle des erreurs 
#     - La normalitÃ© des residus 
#     - HomoscedacitÃ© 
#     - AutocorrÃ©lation 
...
xtable(round(cormat, 4),
    caption = "Correlation des rÃ©sidus",
    align = "l|r" #rrrr"
    )
...
plot(y = as.numeric(ols$res), 
    ...
    col = "red", 
    main = "Demande", 
    ylab = "Fitted - rÃ©sidus", xlab = "Real")
...
lines(y = as.numeric(ols$fit) + as.numeric(ols$res), 
    x = as.numeric(dataWX$qi),
    col = "black")
legend(x = "bottomright", legend = c("RÃ©sidus", "Fitted", "Real"),
...
```{r echo = FALSE}
## Les rÃ©sidus contre la variable prÃ©dite 
# par(mfrow = c(1,2), cex = 0.5)
...
lines(y = fiml$eq[[1]]$fit + fiml$eq[[1]]$res, x = resdata3$qi,
    col = "black")
legend(x = "bottomleft", legend = c("RÃ©sidus", "Fitted", "Real"),
    col = c("red", "blue", "black"), xpd = NA,
    cex = 1, lwd = 3)

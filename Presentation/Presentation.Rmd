...
## Visualisatoin des interdépendances
```{r echo = FALSE}
p1 = datai %>% 
...
print(xtable(round(correlation, 4), 
    ...
    align = "l|cccccc"), caption.placement = "bottom")
```
...
- Limites du modèle 
    - Faible représentation des effets hetérogènes entre les régions (nous estimons seulement des effets moyens)
...
stargazer(pDW, 
    header = FALSE,
    title = "Durbin-Watson test statistics, t-stat",
    summary = F)
```
...
stargazer(BartT,
    header = FALSE,
    summary = FALSE,
    title = "Bartlett heteroscedasticity test, p-valeur")
```
...
stargazer(ShapT,
    header = FALSE,
    summary = FALSE,
    title = "Shapiro-Wilk normality test, p-valeur")
```
...
texreg(list(sls2, wsls2, sls3, fiml),
    ...
    label = "table : 2sls, w2sls, 3sls and fiml", # wls and sur"
...
print(xtable(lr, 
    ...
    align = "l|rrrrr"), caption.placement = "bottom")
```
...
texreg(float.pos = "!htbp", 
    ...
    label = "table : ols, 2sls et 3sls, full information clusters",custom.coef.names = c("D : IP", 
        "D : Revenu c1", "D : Revenu c2","D : Revenu c3", 
        "O : IP", 
        "O : Surface c1", "O : Surface c2", "O : Surface c3", 
        "O : I pésticides c1", "O : I pésticides c2", "O : I pésticides c3"),
    caption = "Modèle économétrique, variable dependante - quantité du vin")
```

...
- Explication de la méthode utilisée
    ...
    - AIDS model 
        ...
        - 3SLS estimator 
        - Use of enxogenous variables as instruments for endogenous price index 
- Limites du modèle 
...
texreg(list(ols, sur),
    custom.model.names = c("OLS", "SUR"))
```
\tiny

# Les résultats 2SLS vs 3SLS
```{r include = FALSE}
# equations
eqdemand = qi ~ 0 + ipi + ri
eqoffer = qi ~ 0 + ipi + si + iki 
...
\tiny
```{r echo = FALSE, results = "asis"}
texreg(list(sls2, sls3),
    custom.model.names = c("2SLS", "3SLS"))
```
\tiny

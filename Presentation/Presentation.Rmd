...
- Le marché commun
2  grandes classes (Cembalo et al, 2014) :
    - le vins de qualité inférieure : peu onéreux, qualité de base -> vins non IGP 
    -les vins de qualité supérieure : onéreux, caractéristiques qualitatives complexe, bonne image et réputation -> vins IGP,AOC.

- Cadre d'étude :
    - Restreint aux vin non IGP (produits relativement homogènes : fourchette de prix étroite,  attributs intrinsèques simples, une complexité de qualité médiocre et donc une différenciation peu marquée.)
    - Restreint au niveau national (non prise en compte des exportations et importations, non prise en compte des marchés extérieurs à la France car différence de taxe sur les produits consommés et perception des produits distinctes)
...
# Les données
- Sources des données : 
     -Les données de ventes de pesticides par département : Institut National de l'Environnement
...
Construction d'une base de données en panel
Double Dimention :
    -individuelle :  les départements français (en France Métropolitaine excepté la Corse)
   - temporelle : les années (2012 à 2016)
...
cordata = dataWX %>% 
    mutate(u1 = ols$eq[[1]]$res,
        u2 = ols$eq[[2]]$res)
cormat = cor(cordata[,-c(6,7)])
colnames(cormat) = c("Vin", "IP", 
        "Surface", "Revenus", 
        "Pesticides", 
        "Demande", "Offre")
...
# Residuals tests for 3SLS
- Residuals normality test (Shapiro-Wilk)
    - Equation de la demande :
```{r echo = FALSE, results = "asis"}
shapiro.test(sls3$eq[[1]]$res)$p.val
```

    - Equation de l'offre :
```{r echo = FALSE, results = "asis"}
shapiro.test(sls3$eq[[2]]$res)$p.val
```

```{r echo = FALSE, results = "asis"}
resdata3 = dataWX %>% 
    mutate(u1 = sls3$eq[[1]]$res,
        u2 = sls3$eq[[2]]$res)
```
- Residuals heteroscedasticity test (Bartlett test)
    - Equation de la demande :
```{r echo = FALSE, results = "asis"}
ols_test_bartlett(resdata3, u1, group_var = ndep)$pval
```

    - Equation de l'offre :
```{r echo = FALSE, results = "asis"}
ols_test_bartlett(resdata3, u2, group_var = ndep)$pval
```

# Residuals autocorrelation study
```{r echo = FALSE}
par(mfrow = c(1,2))
acf(sls3$eq[[1]]$res, main = "Demand")
acf(sls3$eq[[2]]$res, main = "Offre")
```

# Disturbances correlation study for 2SLS 
